    ...
    >
      ...
      <div ref={rowRef} className="relative flex w-full flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3 lg:flex-nowrap lg:px-6">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-3 xl:mr-6 2xl:mr-10">
          ...
          {/* 인사말은 찜·언어 버튼 묶음 전체 위에 얹는다 — 찜 버튼 위에만
              붙였더니 인사말이 왼쪽으로 치우쳐 보였다(피드백). 찜 버튼은
              아이콘만 있으면 뭔지 몰라서 "찜" 글자를 같이 둔다 */}
          <div className="flex flex-col items-end gap-1">
            ...
            <div className="flex items-center gap-3">
              {/* 찜 모아보기 — 명소·코스·표현을 한 곳에서 */}
              <Link
                href="/saved"
                aria-label={t("saved.pageTitle")}
                className="flex h-9 items-center gap-1.5 rounded-full border border-slate-200 px-3.5 text-sm font-semibold text-slate-700 transition hover:border-rose-300 hover:text-rose-500"
              >
                <Heart size={16} />
                {t("nav.savedShort")}
              </Link>
