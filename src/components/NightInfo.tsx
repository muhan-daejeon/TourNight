...
export default async function NightInfo() {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-1.5 rounded-2xl border border-slate-200 bg-slate-100 px-5 py-3 text-sm backdrop-blur">
      {/* 대전 현재 기온·강수 — "오늘 밤 나가도 되나"의 첫 번째 답 */}
      {c.temp !== null && (
        <span className="flex items-center gap-1.5 text-slate-400">
          <Thermometer size={15} className="text-sky-600" />
          {t("tempNow")} <b className="text-slate-900">{c.temp}°C</b>
        </span>
      )}
      {c.precip && (
      ...
      <span className="flex items-center gap-1.5 text-slate-400">
        <Sunset size={15} className="text-amber-600" />
      ...
      {c.fullMoon && (
        <span className="font-semibold text-amber-600">{t("fullMoon")}</span>
      )}
    </div>
  );
}
