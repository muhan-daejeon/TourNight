...
// "성향{문항번호}_{1~4}.jpg"(그 문항의 A~D 선택지 사진, jpg/jpeg/png 아무거나)를
// 넣고 실행하면, public/persona/에 가볍게 리사이즈해 저장하고
// src/lib/persona-images.ts에 타입이 붙은 매니페스트를 만든다.
//
...
const INTRO_RE = /^성향0\.(jpg|jpeg|png)$/i;
const QUESTION_RE = /^성향(\d+)_([1-4])\.(jpg|jpeg|png)$/i;
...
async function main() {
  ...
  for (const file of files) {
    ...
    const [, qNum, optNum] = m; // 세 번째 캡처(확장자)는 안 씀 — sharp가 내용으로 형식을 알아서 읽는다
