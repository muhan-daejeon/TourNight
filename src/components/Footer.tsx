      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
          ...
          <div className="flex items-center gap-2">
            {SOCIALS.map(({ key, Icon }) => (
              <span
                ...
                className="flex size-9 cursor-default items-center justify-center rounded-full border border-slate-200 text-slate-400"
              >
                <Icon size={16} />
              </span>
            ))}
        ...
        <div className="mt-9 border-t border-slate-200 pt-6">
          <p className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
            ...
            <span>
              <b className="text-slate-700">112</b>{" "}
              {t("etiquette.emergency.police")}
            </span>
            <span>
              <b className="text-slate-700">119</b>{" "}
              {t("etiquette.emergency.fire")}
            </span>
            <span>
              <b className="text-slate-700">1330</b>{" "}
              {t("etiquette.emergency.hotline")}
            </span>
          </p>
          <p className="mt-4 text-xs text-slate-400">{t("footer.notice")}</p>
          ...
          <p className="mt-1 text-[11px] text-slate-400">
            지도: Kakao · 경로: SK open API TMAP · ODsay LAB · 날씨: 기상청 ·
            공공자전거: 대전교통공사(타슈) · 캐릭터: 대전관광공사(꿈씨패밀리) ·
            AI: Google Gemini
          </p>
          {/* 공공누리 1유형 출처표시 의무 이행 */}
          <p className="mt-1 text-[11px] text-slate-400">
